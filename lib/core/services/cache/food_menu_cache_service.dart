...
class FoodMenuCacheService {
  ...
  Future<List<Map<String, dynamic>>?> getCachedWeeklyMenus(String ownerId,
      {String? pgId}) async {
    try {
      ...
      final timestampKey =
          '${_timestampPrefix}weekly_${ownerId}_${pgId ?? 'all'}';
      ...
      final menus =
          List<Map<String, dynamic>>.from(jsonDecode(cachedData) as List);
      return menus;
    } catch (e) {
  ...
  /// Cache weekly menus
  Future<void> cacheWeeklyMenus(
      String ownerId, List<Map<String, dynamic>> menus,
      {String? pgId}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
  ...
  Future<List<Map<String, dynamic>>?> getCachedMenuOverrides(String ownerId,
      {String? pgId}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
      ...
      final timestampKey =
          '${_timestampPrefix}overrides_${ownerId}_${pgId ?? 'all'}';
  ...
  Future<void> cacheMenuOverrides(
      String ownerId, List<Map<String, dynamic>> overrides,
      {String? pgId}) async {
    try {
      final prefs = await SharedPreferences.getInstance();
