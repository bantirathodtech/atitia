...
void main() async {
  ...
  if (firebaseAndroidApiKey.isEmpty ||
      firebaseAndroidApiKey.contains('YOUR_')) {
    issues.add('❌ Firebase Android API Key is missing or placeholder');
  } else if (firebaseAndroidApiKey.startsWith('AIza')) {
    print(
        '✅ Firebase Android API Key: ${firebaseAndroidApiKey.substring(0, 20)}...');
  }
  ...
  try {
    ...
    if (androidClientId.contains('YOUR_') ||
        androidClientId.contains('REPLACE_WITH')) {
      ...
      print(
          '✅ Google Android Client ID: ${androidClientId.substring(0, 30)}...');
    } else {
  ...
  try {
    ...
    if (clientSecret.contains('YOUR_') ||
      ...
      print(
          '✅ Google Client Secret: ${clientSecret.substring(0, 10)}... (hidden)');
    } else {
  ...
  if (razorpayApiKey.isEmpty) {
    ...
    warnings.add(
        '⚠️  Razorpay API Key is a TEST key (rzp_test_*). Use production key (rzp_live_*) for production');
    print('⚠️  Razorpay API Key: TEST KEY DETECTED - $razorpayApiKey');
  } else if (razorpayApiKey.startsWith('rzp_live_')) {
    print(
        '✅ Razorpay API Key: PRODUCTION KEY - ${razorpayApiKey.substring(0, 15)}...');
  } else {
  ...
  if (razorpayKeySecret.isEmpty) {
    ...
    print(
        '✅ Razorpay Key Secret: ${razorpayKeySecret.substring(0, 5)}... (hidden)');
  }
